...
RC PlainCommunicator::write_result_internal(SessionEvent *event, bool &need_disconnect)
{
  ...
  if (sql_result->get_table() == nullptr) {
    // 输出 数据  具体的数据细节
    rc           = RC::SUCCESS;
    Tuple *tuple = nullptr;
    ...
    Table                     *table = sql_result->get_table();
    for (int i = 0; i < table->indexes().size(); i++) {
      // 输出 数据  具体的数据细节
      rc                = RC::SUCCESS;
      const char *delim = " | ";
      rc                = writer_->writen(table->name(), strlen(table->name()));
      rc                = writer_->writen(delim, strlen(delim));
      if ((table->indexes())[i]->uniqe_index()) {
        rc = writer_->writen("1", strlen("1"));
        rc = writer_->writen(delim, strlen(delim));
      } else {
        rc = writer_->writen("0", strlen("1"));
        rc = writer_->writen(delim, strlen(delim));
      }
      rc = writer_->writen(table->indexes()[i]->index_meta().name(), strlen(table->indexes()[i]->index_meta().name()));
      rc = writer_->writen(delim, strlen(delim));

      auto temp = (table->indexes())[i]->field_meta().name();
      auto iter = personnel.find(std::string(temp));

      if (iter != personnel.end()) {
        std::string id = std::to_string(++iter->second);
        rc             = writer_->writen(id.c_str(), id.size());
      } else {
        personnel.insert(std::pair<std::string, int>(temp, 1));
        rc = writer_->writen("1", strlen("1"));
      }
      rc          = writer_->writen(delim, strlen(delim));
      auto temp_1 = (table->indexes())[i]->field_meta().name();
      rc          = writer_->writen(temp_1, strlen(temp_1));
