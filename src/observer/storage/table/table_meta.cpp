...
RC TableMeta::init(int32_t table_id, const char *name, int field_num, const AttrInfoSqlNode attributes[])
{
  ...
  for (int i = 0; i < field_num; i++) {
    ...
    rc                               = fields_[i + trx_field_num].init(
        attr_info.name.c_str(), attr_info.type, field_offset, attr_info.length, true /*visible*/,attr_info.can_be_null);
    if (rc != RC::SUCCESS) {
...
const FieldMeta *TableMeta::field_num(int num) const { return &fields_[num]; }
int              TableMeta::find_offset_by_name(std::string att_name)
{
  for (const FieldMeta &field : fields_) {
    if (field.name_s() == att_name) {
      return field.offset();
    }
  }
  return -1;
...
AttrType TableMeta::find_att_by_name(std::string att_name)
{
  for (const FieldMeta &field : fields_) {
    if (field.name_s() == att_name) {
...
int TableMeta::find__attry(std::string att_name)
{
  for (const FieldMeta &field : fields_) {
    if (field.name_s() == att_name) {
      return 1;
...
// 使用这个函数时一定要确定name存在
int TableMeta::find_index_by_name(std::string att_name) const
{
  int i = 0;
  for (const FieldMeta &field : fields_) {
    if (field.name_s() != att_name) {
...
int TableMeta::find_att_len(std::string att_name)
{
  for (const FieldMeta &field : fields_) {
    if (field.name_s() == att_name) {
      return field.len();
