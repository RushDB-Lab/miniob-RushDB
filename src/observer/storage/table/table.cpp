...
RC Table::SavetoFile()
{
  std::string   data_location = my_table_file(base_dir_.c_str(), name());
  std::ofstream file(data_location, std::ios::binary);
  if (file) {
    // 写入myInt的值
    file.write(reinterpret_cast<const char *>(&row_num), sizeof(row_num));
    ...
    // // 写入myVector的数据
    // file.write(reinterpret_cast<const char *>(myVector.data()), vectorSize * sizeof(int));
...
RC Table::LoadtoFile()
{
  std::ifstream file(my_table_file(base_dir_.c_str(), name()));
  if (file) {
    // 读取myInt的值
    file.read(reinterpret_cast<char *>(&row_num), sizeof(row_num));
    ...
    // file.read(reinterpret_cast<char *>(myVector.data()), vectorSize * sizeof(int));
...
RC Table::update_record(const Record &record, const char *data, std::string att_name)
{
  RC rc = RC::SUCCESS;
  // 先更新record的值
  int a1 = find_att_len(att_name), a2 = find_offset(att_name);
  rc = record_handler_->update_record(data, a1, &record.rid(), a2);
  // 再更新record所对应的索引
...
RC Table::make_record(int value_num, const Value *values, Record &record)
{
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    if (field->type() == CHARS || field->type() == null) {
...
RC Table::create_index(Trx *trx, const FieldMeta *field_meta, const char *index_name, bool unique_index_)
{
  ...
  index_field_dir.push_back(index_file);
  rc = index->create(index_file.c_str(), new_index_meta, *field_meta, unique_index_);
  if (rc != RC::SUCCESS) {
    delete index;
...
int Table::find_offset(std::string att_name) { return table_meta_.find_offset_by_name(att_name); }
int Table::find_att_len(std::string att_name) { return table_meta_.find_att_len(att_name); }
...
int      Table::find_attry_name(std::string att_name) { return table_meta_.find__attry(att_name); }
AttrType Table::find_attry_by_name(std::string name) { return table_meta_.find_att_by_name(name); }
...
const FieldMeta *Table::find_fieldmate_by_name(const char *name) { return table_meta_.field(name); }

RC Table::destroy_index()
{
  for (auto index : indexes_) {
    delete index;
  }
  for (auto dir : index_field_dir) {
    remove(dir.c_str());
  }
  indexes_.clear();
  index_field_dir.clear();
  return RC::SUCCESS;
}
