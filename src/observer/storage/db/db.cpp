...
RC Db::create_table(const char *table_name, int attribute_count, const AttrInfoSqlNode *attributes)
{
  ...
  // rc = table->create(next_table_id_++, table_file_path.c_str(), table_name, path_.c_str(), attribute_count,
  // attributes);
  if (rc != RC::SUCCESS) {
...
RC Db::drop_table(std::string tablename)
{
  RC     rc     = RC::SUCCESS;
  Table *droped = find_table(tablename.c_str());
  if (droped == nullptr) {
    return RC::FILE_NOT_EXIST;
  }
  // 删除索引
  droped->destroy_index();
  // 在database的管理实例里删除
  opened_tables_.erase(tablename);

  // 在buffer pool里面先关闭

  if (droped->get_data_buffer_pool_() != nullptr) {
    droped->data_buffer_pool_->close_file();
    droped->get_data_buffer_pool_()->close_file();  // create table tess(age int ,name char)
  }
  // 删除表原和数据
  std::string dir    = droped->get_dir();
  std::string moved1 = dir + '/' + tablename + ".table";
  std::string moved2 = dir + '/' + tablename + ".data";
  if (std::remove(moved1.c_str()) != 0)
    return RC::FILE_NOT_EXIST;
  if (std::remove(moved2.c_str()) != 0)
    return RC::FILE_NOT_EXIST;
