...
{
  [[maybe_unused]] RC rc = this->init(name, attr_type, attr_offset, attr_len, visible, can_has_null);
  ASSERT(rc == RC::SUCCESS, "failed to init field meta. rc=%s", strrc(rc));
}

RC FieldMeta::init(
    const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, bool can_has_null_)
{
...
std::string FieldMeta::name_s() const { return name_; }
...
void FieldMeta::to_json(Json::Value &json_value) const
{
  ...
  json_value[FIELD_NULLABLE] = can_has_null;
}
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  bool        can_be_null_ = nullable_value.asBool();
  return field.init(name, type, offset, len, visible, can_be_null_);
}
