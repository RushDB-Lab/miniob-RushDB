...
#include "sql/operator/orderby_operator/order_by_physical_operator.h"
#include "sql/operator/show_index_logical_operator.h"
#include "sql/operator/show_index_physical_operator.h"
...
RC PhysicalPlanGenerator::create(LogicalOperator &logical_operator, unique_ptr<PhysicalOperator> &oper)
{
  ...
  switch (logical_operator.type()) {
    ...
    case LogicalOperatorType::AGG_FUNC: {
      return create_plan(static_cast<AggFuncLogicalOperator &>(logical_operator), oper);
    } break;
    ...
    case LogicalOperatorType::SHOW_INDEX: {
      return create_plan(static_cast<ShowIndexLogicalOperator &>(logical_operator), oper);
    }
    default: {
...
RC PhysicalPlanGenerator::create_plan(AggFuncLogicalOperator &Agg_Func_oper, std::unique_ptr<PhysicalOperator> &oper)
{
  vector<unique_ptr<Expression>> &predicates = Agg_Func_oper.predicates();
  // 看看是否有可以用于索引查找的表达式
  Table *table = Agg_Func_oper.table();
  ...
  auto table_scan_oper = new AggFuncPhysicalOperator(
      table, Agg_Func_oper.readonly(), Agg_Func_oper.get_agg_func(), Agg_Func_oper.get_files());
...
RC PhysicalPlanGenerator::create_plan(PredicateLogicalOperator &pred_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  std::vector<std::unique_ptr<LogicalOperator>> child_logical_operator =
      std::move(pred_oper.get_child_select_logical_operator_());

  std::vector<std::unique_ptr<PhysicalOperator>> child_select_physical_operator_;
  for (int i = child_logical_operator.size() - 1; i >= 0; i--) {
    std::unique_ptr<PhysicalOperator> child_oper;
    create(*child_logical_operator[i], child_oper);
    child_select_physical_operator_.push_back(std::move(child_oper));
  }

  unique_ptr<Expression> expression = std::move(expressions.front());
  oper                              = unique_ptr<PhysicalOperator>(
      new PredicatePhysicalOperator(std::move(expression), child_select_physical_operator_));
...
RC PhysicalPlanGenerator::create_plan(ShowIndexLogicalOperator &show_index_oper, unique_ptr<PhysicalOperator> &oper)
{
  RC                         rc                  = RC::SUCCESS;
  ShowIndexPhysicalOperator *show_index_operator = new ShowIndexPhysicalOperator;
  const vector<Index *>     &project_indexes     = show_index_oper.indexes();
  for (auto &index : project_indexes) {
    show_index_operator->add_projection(show_index_oper.table(), &index->index_meta());
  }
  oper = unique_ptr<PhysicalOperator>(show_index_operator);
...
RC PhysicalPlanGenerator::create_plan(OrderByLogicalOperator &orderby_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  auto orderbys = orderby_oper.get_OrderBy();
  orderby_oper.get_OrderBy().clear();
  OrderByPhysicalOperator *orderby_phy_oper = new OrderByPhysicalOperator();
  orderby_phy_oper->init_compator(orderbys);
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  oper = unique_ptr<PhysicalOperator>(
      new UpdatePhysicalOperator(update_oper.table(), update_oper.name(), std::move(values)));
