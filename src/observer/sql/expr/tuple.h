...
public:
  friend class AggFuncPhysicalOperator;
  RowTuple() = default;
  RowTuple *clone() const override { return new RowTuple(*this); }
  RowTuple(RowTuple const &other)
  {
  ...
  RC cell_at(int index, Value &cell) const override
  {
    ...
    const char *set_data = this->record_->data() + field_meta->offset();
    cell.set_data(set_data, field_meta->len());
    if (std::memcmp(set_data, "null", 4) == 0) {
      // 这里是判断是否为null，如果是null，那么就设置为null
      cell.set_null();
    }
    return RC::SUCCESS;
  }
  ...
  RC find_cell(const TupleCellSpec &spec, Value &cell) const override
  {
    ...
    for (size_t i = 0; i < speces_.size(); ++i) {
      if (is_visited.size() != 0 && is_visited[i])
        continue;
  ...
  void set_fieldexper(std::vector<FieldExpr *> field_exprs) { speces_ = field_exprs; }
...
private:
  mutable std::vector<bool> is_visited;
...
public:
  ...
  ProjectTuple *clone() const override { return new ProjectTuple(*this); }
  ProjectTuple(ProjectTuple const &other)
  {
...
public:
  ...
  ExpressionTuple *clone() const override { return new ExpressionTuple(*this); }
...
public:
  ...
  ValueListTuple *clone() const override { return new ValueListTuple(*this); }
...
public:
  JoinedTuple() = default;
  JoinedTuple *clone() const override { return new JoinedTuple(*this); }
  JoinedTuple(JoinedTuple const &other)
  {
