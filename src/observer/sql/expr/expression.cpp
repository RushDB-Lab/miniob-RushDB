...
RC ComparisonExpr::compare_value(const Value &left, const Value &right, bool &result) const
{
  ...
  result         = false;
  if (left.attr_type() == null || right.attr_type() == null) {
    if (comp_ == is || comp_ == is_not) {
      goto end;
    }
    result = false;
    return rc;
...
RC ComparisonExpr::try_get_value(Value &cell) const
{
  if (left_->type() == ExprType::VALUE && right_->type() == ExprType::VALUE) {
    ...
    auto         k                = (left_cell.attr_type() == null ^ right_cell.attr_type() == null);
    if (left_cell.attr_type() == null && right_cell.attr_type() == null) {
      if (comp_ == is) {
        cell.set_boolean(true);
      } else {
        cell.set_boolean(false);
      }
      return RC::SUCCESS;
    } else if (k) {
      if (comp_ == is_not) {
