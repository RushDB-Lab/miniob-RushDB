...
RC check_aggregate_expression(AggregateFunctionExpr &expression)
{
  ...
  // 校验数据类型与聚合类型是否匹配
  builtin::AggregateFunctionType aggregate_type   = expression.aggregate_type();
    ...
    unique_ptr<Expression> &expr, vector<unique_ptr<Expression>> &bound_expressions)
{
  ...
  builtin::AggregateFunctionType aggregate_type;
  ...
  if (OB_SUCC(rc)) {
    ...
    if (child_expr->type() == ExprType::STAR && aggregate_type == builtin::AggregateFunctionType::COUNT) {
  ...
  builtin::NormalFunctionType func_type;
