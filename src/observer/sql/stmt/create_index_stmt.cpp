...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  for (int i = create_index.attribute_names.size() - 1; i >= 0; i--) {
    if (is_blank(table_name) || is_blank(create_index.index_name.c_str()) ||
        is_blank(create_index.attribute_names[i].c_str())) {
  ...
  std::string      attribute_name = create_index.attribute_names[0];
  ...
  stmt = new CreateIndexStmt(table, field_meta, create_index.index_name, create_index.unique_index);
  return RC::SUCCESS;
}