 ...
 */
class AggFuncLogicalOperator : public LogicalOperator
{
public:
  AggFuncLogicalOperator(
      Table *table, const std::vector<Field> &fields, std::vector<Agg_Func> &agg_func_, bool readonly);
  virtual ~AggFuncLogicalOperator() = default;

  LogicalOperatorType type() const override { return LogicalOperatorType::AGG_FUNC; }

  Table *table() const { return table_; }
  auto   get_agg_func() { return agg_funcs; }
  auto   get_files() { return fields_; }
  bool   readonly() const { return readonly_; }

  void  creat_predicates_(FilterStmt *filter_stmt);
